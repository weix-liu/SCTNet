...
class Visualize:
    def __init__(self, root: str, device: str):
        ...
        self.models = {
            ...
            # "mcdnet": MCDNet(in_channels=3, num_classes=self.num_classes).to(self.device),
            # "scnn": SCNN(num_classes=self.num_classes).to(self.device),
            # "dbnet": DBNet( img_size=self.image_size, in_channels=3, num_classes=self.num_classes ).to(self.device),

            "unetmobv2": UNetMobV2(num_classes=self.num_classes).to(self.device),
            "kappamask": KappaMask(num_classes=self.num_classes, in_channels=3).to(self.device),
            "sctnet": SCTNet(in_channels=3, num_classes=self.num_classes).to(self.device)
        }
        ...
        self.weights_path = {
            "kappamask": "/data/weix/SCTNet/logs/l8_biome_crop/kappamask/2024-12-23_18-47-31/checkpoints/step_40000.ckpt",
            "unetmobv2": "/data/weix/SCTNet/logs/l8_biome_crop/unetmobv2/2024-12-23_17-14-37/checkpoints/step_24000.ckpt",
            "sctnet": "/data/weix/SCTNet/logs/l8_biome_crop/sctnet/savemIoU6603/checkpoints/6603step_32000.ckpt",
        }
    ...
    def __load_weight(self):
        ...
        for model_name, model in self.models.items():
            ...
            # )[0]
            weight_path = self.weights_path[model_name]
    ...
    def visualize_img_no_text(
        ...
        show_image = torchvision.utils.make_grid(show_images_tensor, nrow=1, padding=10)
        grid_image = torchvision.transforms.ToPILImage()(show_image)
