...
class CloudSEN12High(Dataset):
    ...
    def __init__(
        ...
        self.image_paths, self.label_paths = self.load_data()

    def load_data(self):

        mask_paths = glob(os.path.join(self.root, self.level, 'ann_dir', self.phase, "*.png"))
        image_paths = [filename.replace('ann_dir','img_dir') for filename in mask_paths]
    ...
    def __getitem__(self, idx):
        ...
        img = img /256. # 1e-6 to avoid division by zero

        # mean = [123.675, 116.28, 103.53]
        # std = [58.395, 57.12, 57.375]
        # img = (img - np.array(mean).astype(np.float32).reshape(-1, 1, 3))/np.array(std).astype(np.float32).reshape(-1, 1, 3)
